#include "../ft_printf.h"

static void	precision_case(t_printff *const fl, const size_t len, const uintmax_t val)
{
	fl->flags[5] = (unsigned char)(fl->precision == -1);
	if (fl->flags[0] && val && fl->precision <= (long long)len)
		fl->precision = (long long int)(len) + 1;
	if (fl->flags[0] && (!val) && fl->precision == 0)
		fl->precision = 1;
	if (fl->precision != -1)
	{
		if (!fl->flags[5])
			fl->flags[1] = 0;
	}
	else
	if (fl->width > 0 && fl->flags[1])
		fl->precision = (long long int)fl->width;
}

/*
**	Here, fl.flag[5] is special flag shows was there a presicion in call
*/

void ftprt_put_o(t_printff *const fl, va_list *const arg)
{
	const uintmax_t     val = ftprt_va_get_uvalue(fl, arg);
	const size_t        len = ftprt_unumber_baselen(val,
		(unsigned char) ((fl->precision != 0) ? 1 : fl->flags[0]),
		fl->flags[4], 8);
	size_t              used_len;


	precision_case(fl, len, val);
	used_len = (fl->precision == -1 || len >
		(long long unsigned)fl->precision) ? len : (size_t)fl->precision;
	if (used_len < fl->width && (!fl->flags[2]))
		ftprt_putnchar(fl, (char)((fl->flags[1]) ? '0' : ' '),
					   fl->width - used_len);
	ftprt_put_onumber(val, len, fl);
	if (used_len < fl->width && fl->flags[2])
		ftprt_putnchar(fl, ' ', fl->width - used_len);
	fl->count += ft_max_size_t(fl->width, used_len);
}